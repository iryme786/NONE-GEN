...
@web.get("/") def home(): return {"status": "✅ Bot is running", "message": "Use this bot on Telegram"}

@app.on_message(filters.command("start")) async def start(client, message: Message): await message.reply("📧 Please enter your email address:") user_state[message.from_user.id] = "WAITING_FOR_EMAIL"

@app.on_message(filters.text & ~filters.command("start")) async def handle_input(client, message: Message): user_id = message.from_user.id if user_id not in user_state: return await message.reply("❗ Type /start to begin.")
...
threading.Thread(target=run).start() uvicorn.run(web, host="0.0.0.0", port=10000)
